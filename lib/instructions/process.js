...
function processInstructionsDirectory() {
    var instructionsFileMatcher = global.instructionsPath + '/' + global.instructionsGlob;
    return find(instructionsFileMatcher)
        .then(checkFiles)
        .then(loadFiles);
}
...
function checkFiles(files) {
    if (files && files.length > 0) {
        ...
        return Promise.reject(['No instructions found', global.instructionsPath + '/' + global.instructionsGlob].join(', '));
    }
}
